...
{
    WVJSCoreBridge* bridge = [[WVJSCoreBridge alloc] init];
...
- (void)_dispatchMessage:(WVJBMessage*)message {
    ...
    NSString* javascriptCommand = [NSString stringWithFormat:@"WVJSCoreBridge._handleMessageFromObjC('%@');", messageJSON];
...
- (void)_flushMessageQueue {
    NSString *messageQueueString = [_webView stringByEvaluatingJavaScriptFromString:@"WVJSCoreBridge._fetchQueue();"];
    
    id messages = [self _deserializeMessageJSON:messageQueueString];
    if (![messages isKindOfClass:[NSArray class]]) {
        NSLog(@"WVJSCoreBridge: WARNING: Invalid %@ received: %@", [messages class], messages);
        return;
    }
    for (WVJBMessage* message in messages) {
        if (![message isKindOfClass:[WVJBMessage class]]) {
            NSLog(@"WVJSCoreBridge: WARNING: Invalid %@ received: %@", [message class], message);
            continue;
        }
        ...
        handler(message[@"handlerName"],message[@"data"], responseCallback);
    }
}
...
- (void)webView:(WebView *)webView didFinishLoadForFrame:(WebFrame *)frame
{
    ...
    if (![[webView stringByEvaluatingJavaScriptFromString:@"typeof WVJSCoreBridge == 'object'"] isEqualToString:@"true"]) {
        NSBundle *bundle = _resourceBundle ? _resourceBundle : [NSBundle mainBundle];
        NSString *filePath = [bundle pathForResource:@"WVJSCoreBridge.js" ofType:@"txt"];
...
- (void)webView:(WebView *)webView decidePolicyForNavigationAction:(NSDictionary *)actionInformation request:(NSURLRequest *)request frame:(WebFrame *)frame decisionListener:(id<WebPolicyDecisionListener>)listener
{
    ...
    if ([[url scheme] isEqualToString:kCustomProtocolScheme]) {
        if ([[url host] isEqualToString:kQueueHasMessage]) {
            ...
            NSLog(@"WVJSCoreBridge: WARNING: Received unknown WVJSCoreBridge command %@://%@", kCustomProtocolScheme, [url path]);
        }
        [listener ignore];
...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
    ...
    if (_numRequestsLoading == 0 && ![[webView stringByEvaluatingJavaScriptFromString:@"typeof WVJSCoreBridge == 'object'"] isEqualToString:@"true"]) {
        NSBundle *bundle = _resourceBundle ? _resourceBundle : [NSBundle mainBundle];
        NSString *filePath = [bundle pathForResource:@"WVJSCoreBridge.js" ofType:@"txt"];
        NSString *js = [NSString stringWithContentsOfFile:filePath encoding:NSUTF8StringEncoding error:nil];
        [webView stringByEvaluatingJavaScriptFromString:js];
